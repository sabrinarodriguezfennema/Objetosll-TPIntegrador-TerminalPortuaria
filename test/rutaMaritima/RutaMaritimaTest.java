...
public class RutaMaritimaTest {
	...
	void setUp() {
		...
		when(viaje.getCircuito()).thenReturn(circuito);
		when(viaje.cronograma()).thenReturn(Map.of(terA, LocalDateTime.of(2025, 11, 10, 0, 0), terB,
				LocalDateTime.of(2025, 11, 13, 0, 0), terC, LocalDateTime.of(2025, 11, 16, 0, 0)));
	...
	@Test
	void fechaSalida() {
		assertEquals(LocalDate.of(2025, 11, 10), ruta.fechaSalida().toLocalDate());
	}

	@Test
	void fechaLlegada() {
		assertEquals(LocalDate.of(2025, 11, 16), ruta.fechaLlegada().toLocalDate());
	}
